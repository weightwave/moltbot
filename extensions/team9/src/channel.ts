 ...
 */
async function getConnection(account: ResolvedTeam9Account, cfg: MoltbotConfig) {
  ...
  const ws = createTeam9WsClient(
    ...
    {
      onMessage: (message) => {
        // Forward message to Moltbot agent for processing
...
export const team9Plugin: ChannelPlugin<ResolvedTeam9Account> = {
  ...
  // Onboarding adapter for `moltbot onboard` wizard
